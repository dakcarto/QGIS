...
void O2ReplyServer::onBytesReady() {
    ...
    reply.append(QString("Content-Length: %1\r\n\r\n").arg(replyContent_.size()).toAscii());
...
QMap<QString, QString> O2ReplyServer::parseQueryParams(QByteArray *data) {
    ...
    foreach (tokenPair, tokens) {
        ...
        QString key = QUrl::fromPercentEncoding(QByteArray().append(tokenPair.first.trimmed().toAscii()));
        QString value = QUrl::fromPercentEncoding(QByteArray().append(tokenPair.second.trimmed().toAscii()));
